...
(function () {
  ...
  /* --- Case study channel-asset crossfade --------------------------------- */
  /* The active slide ships with .is-active in the markup, so no-JS and
     reduced-motion both render a correct, static frame. This only adds the
     rotation, and only when there is more than one real slide to rotate to. */
  (function channelFade() {
    var frames = document.querySelectorAll('.channel-fade__frame');
    if (!frames.length) return;

    frames.forEach(function (frame) {
      var slides = Array.prototype.slice.call(frame.querySelectorAll('.channel-fade__slide'));
      var dots = Array.prototype.slice.call(frame.parentElement.querySelectorAll('.channel-fade__dots span'));
